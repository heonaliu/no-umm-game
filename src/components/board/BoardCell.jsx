...
export function BoardCell({ position, type, pawns = [], isActive = false }) {
  return (
    <div className={clsx(
      ...
      type === "normal" && "bg-sky-50/70   border-sky-100",
      isActive && "ring-2 ring-sky-400 ring-offset-1 ring-offset-white",
    )}>
      <span className="absolute top-0.5 left-1 text-[8px] font-bold text-sky-300 leading-none">
        {position}
      </span>
      <CellIcon type={type} />
      {pawns.length > 0 && (
