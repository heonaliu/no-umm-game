/**
 * GameBoard — snake-layout board.
 * Row 0 (bottom) = spaces 0–9, row 1 = 10–19 reversed, etc.
 * Auto-scrolls to keep the active pawn centred.
 */
...
export function GameBoard() {
  ...
  const rows = useMemo(() => {
    ...
    for (let r = 0; r < rowCount; r++) {
      ...
      if (r % 2 === 1) row = [...row].reverse(); // snake
      builtRows.push(row);
    }
    return builtRows.reverse(); // bottom = start
  }, [totalCells]);
  ...
  return (
    <div className="w-full rounded-2xl border border-indigo-100 bg-white shadow-sm p-4">
      {/* Legend */}
      <div className="flex items-center gap-4 mb-3 flex-wrap">
        <div className="flex items-center gap-1.5 text-xs text-amber-700">
          <Star size={13} className="fill-amber-400 text-amber-500" />
          <span>Rule Space (every {YELLOW_INTERVAL}th)</span>
        </div>
        <div className="flex items-center gap-1.5 text-xs text-red-600">
          <Flame size={13} />
          <span>Danger Zone (last {DANGER_ZONE_SIZE})</span>
        </div>
      </div>
      ...
      {/* Team positions legend */}
      <div className="mt-3 pt-3 border-t border-indigo-50 flex gap-3 flex-wrap">
        {teams.map((team, i) => (
          <div key={team.id} className="flex items-center gap-1.5 text-xs">
            ...
            <span className="text-indigo-300">·{team.position}</span>
            {i === currentTeamIndex && (
              <span className="text-indigo-500 font-bold text-[10px] bg-indigo-100 px-1.5 py-0.5 rounded-full">
                ACTIVE
              </span>
            )}
          </div>
